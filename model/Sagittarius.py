...
import numpy as np
from typing import *
from model._base_components import DecoderSCVI
from model._negative_binomial import ZeroInflatedNegativeBinomial
...
class Sagittarius(nn.Module):
    def __init__(self, input_dim: int, num_classes: int, class_sizes: List[int],
                 ...
                 rec_loss: str='mse', use_transformer: bool = False):
    ...
    def forward(self, xs: Tensor, ts: Tensor, ys: List[Tensor], mask: Tensor, other_ts: List[Tensor] = None) -> Tuple[Tensor]:
        ...
        for tensor in cvae_ys:
            reduced_tensor = torch.mean(tensor.to(torch.float32), dim=2)
        cvae_ys = []
        cvae_ys.append(reduced_tensor)
        ...
        z_dec = self.cvae_enc.decode(torch.cat([z_cvae, *cvae_ys], dim=-1).view(N*T, -1)).view(N, T, M)

        return z_dec, mu, logvar
...
class cVAE(nn.Module):
    ...
    def __init__(self,
        ...
        print("h")
        self.decoder2 = DecoderSCVI(
            5731, # M = # genes, dimensionality of input
            5731, # dimensionality of output
            n_cat_list=[2], # have this stuff match previous training
            n_layers=2, # number of hidden layers
            n_hidden=[128, 64], # num nodes per hidden layer
            inject_covariates= False, #deeply_inject_covariates,
            use_batch_norm= False, #use_batch_norm_decoder,
            use_layer_norm= False, # use_layer_norm_decoder,
            scale_activation="softmax", #if use_size_factor_key else "softmax",
            **_extra_decoder_kwargs, #literally like i donrt
        )
    ...
    def encode(self, x: Tensor) -> List[Tensor]:
        ...
        Returns:
            ...
            logvar (Tensor): log variance of gaussian distribution
        """
        # print("Shape of x inside encode:", x.shape)
        x = self.encoder(x)
    ...
    def loss_fn(self, pred_y, y, mu, log_var, beta, **kwargs) -> dict:
        ...
        if rec_loss_type == 'mse':
            ...
            recons_y_loss = -zinb.log_prob(y).sum(-1)
